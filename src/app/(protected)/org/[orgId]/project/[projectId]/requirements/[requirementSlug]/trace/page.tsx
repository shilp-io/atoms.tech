...
import { Check, Plus, Trash2, Search, Trash, Network } from 'lucide-react';
// eslint-disable-next-line @typescript-eslint/no-unused-vars
...
import { useCreateTraceLinks, useDeleteTraceLink } from '@/hooks/mutations/useTraceLinkMutations';
import { useDocumentRequirements, useRequirementsByIds } from '@/hooks/queries/useRequirement';
...
export default function TracePage() {
    ...
    const { data: requirements, isLoading: isLoadingRequirements } = useDocumentRequirements(documentId);
    ...
    const availableRequirements = requirements?.filter(
        (req) => {
            ...
            const isSourceInOutgoing = outgoingLinks?.some(link => link.target_id === req.id);
            const isTargetInIncoming = incomingLinks?.some(link => link.source_id === req.id);
            ...
            <Card className="bg-background shadow-none">
                <CardHeader className="px-6 py-4">
                    <div className="flex justify-between items-center">
                        <div>
                            <CardTitle className="text-2xl font-bold">Trace Links</CardTitle>
                            <CardDescription>
                            ...
                            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                                ...
                                <DialogContent className="sm:max-w-[600px] max-h-[80vh] bg-background shadow-none">
                                    ...
                                    <div className="grid gap-4 py-4">
                                        ...
                                        <div className="flex justify-between items-center">
                                            <h4 className="font-medium">Available Requirements</h4>
                                        ...
                                        {isLoadingRequirements ? (
                                            ...
                                            <div className="max-h-[200px] overflow-y-auto">
                                                {availableRequirements.map((req) => {
                                                    const isSelected = selectedRequirements.some(selected => selected.id === req.id);
                                                    return (
                                                        <div key={req.id} className="p-3 flex justify-between items-start hover:bg-muted">
                                                            ...
                                                            {isSelected ? (
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    <Select
                                                                        onValueChange={(value: TraceRelationship) => {
                                                ...
                                                <Table>
                                                    ...
                                                    <TableBody>
                                                        {selectedRequirements.map((req) => (
                                                            <TableRow key={req.id}>
                                                                <TableCell>
                                                                    {req.relationship === 'parent_of' ? 'Parent of' : 'Child of'}
                                                                </TableCell>
                                                                <TableCell className="font-medium">{req.external_id || req.id.split('-')[1]}</TableCell>
                                    ...
                                    <Button 
                                        ...
                                        disabled={selectedRequirements.length === 0 || createTraceLinksMutation.isPending}
                                    >
                                        {createTraceLinksMutation.isPending ? 'Creating...' : 'Create Trace Links'}
                                    </Button>
                                    </DialogFooter>
                                </DialogContent>
                            </Dialog>
                    ...
                    <div className="space-y-6">
                        <div>
                            <div className="max-h-[400px] overflow-y-auto">
                                {(incomingLinks && incomingLinks.length > 0) || (outgoingLinks && outgoingLinks.length > 0) ? (
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                                <TableHead className="w-[100px]">ID</TableHead>
                                                ...
                                                <TableHead className="w-[120px]">Relationship</TableHead>
                                                {isDeleteMode && <TableHead className="w-[80px]">Action</TableHead>}
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody>
                                            {incomingLinks && incomingLinks.map((link) => {
                                                const requirement = linkedRequirements?.find(req => req.id === link.source_id);
                                                return (
                                                    <TableRow key={link.id}>
                                                        <TableCell className="font-medium">{requirement?.external_id || link.source_id.split('-')[1]}</TableCell>
                                                        <TableCell>{requirement?.name || 'Loading...'}</TableCell>
                                                        ...
                                                        {isDeleteMode && (
                                                            <TableCell>
                                                                <Button 
                                                                    ...
                                                                    onClick={() => handleDeleteTraceLink(link.id)}
                                                                    disabled={deleteTraceLinkMutation.isPending}
                                                                >
                                            ...
                                            {outgoingLinks && outgoingLinks.map((link) => {
                                                const requirement = linkedRequirements?.find(req => req.id === link.target_id);
                                                return (
                                                    <TableRow key={link.id}>
                                                        <TableCell className="font-medium">{requirement?.external_id || link.target_id.split('-')[1]}</TableCell>
                                                        <TableCell>{requirement?.name || 'Loading...'}</TableCell>
