  ...
  return useMutation({
    mutationFn: async (kv: Partial<PropertyKeyValue>) => {
      ...
      if (kv.position === undefined) {
        console.warn('⚠️ Position not provided for property KV, defaulting to 0');
        kv.position = 0;
      }
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({
        queryKey: queryKeys.requirementPropertyKVs.byRequirement(data.requirement_id),
      });
    },
  });
}
  ...
  return useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        if (fetchError) {
          console.error('❌ Error fetching existing KVs:', fetchError);
          throw fetchError;
        }
        
        console.log('✅ Fetched existing KVs', { count: existingKVs?.length });
        
        // Create or update KVs for each property in the data object
        const kvPromises = Object.entries(data).map(async ([key, value], index) => {
          const existingKV = existingKVs?.find(kv => kv.property_name === key);
          
          if (existingKV) {
            // Update existing KV if value changed
            if (existingKV.property_value !== String(value)) {
              console.log('🔄 Updating existing KV', { key, value, existingValue: existingKV.property_value });
        ...
        console.log('✅ All KVs synced successfully', { count: results.length });
        
        // Invalidate queries to ensure UI is updated
        queryClient.invalidateQueries({
          queryKey: queryKeys.requirementPropertyKVs.byRequirement(requirementId),
        });
        
        return results;
      } catch (error) {
        console.error('❌ Error syncing requirement data with KVs:', error);
        throw error;
      }
    },
  });
