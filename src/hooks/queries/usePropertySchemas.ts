...
import { BlockPropertySchema, DocumentPropertySchema, PropertyKeyValue } from '@/components/custom/BlockCanvas/types';
...
export function useBlockPropertySchemas(blockId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error('❌ Error fetching block property schemas:', error);
        throw error;
      }
...
export function useCreateBlockPropertySchema() {
  ...
  return useMutation({
    mutationFn: async (schema: Partial<BlockPropertySchema>) => {
      ...
      if (error) {
        console.error('❌ Error creating block property schema:', error);
        throw error;
      }
...
// Property Key Values
export function useRequirementPropertyKVs(blockId: string, requirementId?: string) {
  return useQuery({
    queryKey: queryKeys.requirementPropertyKVs.byBlockAndRequirement(blockId, requirementId),
    queryFn: async () => {
      let query = supabase
