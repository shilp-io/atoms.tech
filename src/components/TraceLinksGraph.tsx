...
import { useTraceLinks, useReverseTraceLinks } from '@/hooks/queries/useTraceability';
import { useRequirementsByIds } from '@/hooks/queries/useRequirement';
...
const TraceLinksGraph = ({ requirementId }: TraceLinksGraphProps) => {
    ...
    // Fetch trace links
    const { data: outgoingLinks, isLoading: outgoingLoading } = useTraceLinks(requirementId, 'requirement');
    const { data: incomingLinks, isLoading: incomingLoading } = useReverseTraceLinks(requirementId, 'requirement');
    ...
    const { data: linkedRequirements, isLoading: requirementsLoading } = useRequirementsByIds(linkedRequirementIds);
    ...
    const createNode = useCallback((entity: Entity, x: number, y: number): Node => {
        ...
        return {
            ...
            data: { 
                label: (
                    <div title={`Click to view requirement details for: ${entity.name || entity.id}`}>
    ...
    useEffect(() => {
        ...
        if (parentNodes.length > 0) {
            ...
            parentNodes.forEach((parentId, index) => {
                ...
                if (parent) {
                    const x = -parentWidth / 2 + index * (nodeWidth + nodeSpacing) + nodeWidth / 2;
        ...
        if (childNodes.length > 0) {
            ...
            childNodes.forEach((childId, index) => {
                ...
                if (child) {
                    const x = -childWidth / 2 + index * (nodeWidth + nodeSpacing) + nodeWidth / 2;
        ...
        setNodes(nodesArray);
        setEdges(edgesArray);
    }, [outgoingLinks, incomingLinks, linkedRequirements, requirementId, currentRequirementData, createNode, setEdges, setNodes]);
