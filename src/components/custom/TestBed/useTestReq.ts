...
import { TestReq, RequirementTest } from '@/components/custom/TestBed/types';
...
export function useProjectTestCases(
    ...
    pagination = { page: 1, pageSize: 10, orderBy: 'created_at', orderDirection: 'desc' as 'asc' | 'desc' }
) {
    return useQuery({
  ...
  return useMutation({
    mutationFn: async (newTestReq: {
      ...
      const testData = {
        ...
        is_active: typeof newTestReq.is_active === 'boolean' ? newTestReq.is_active : true,
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(
        queryKeys.testReq.detail(data.id!),
        data
      );
    },
  });
...
export function useCreateRequirementTest() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [...queryKeys.requirements.detail(data.requirement_id), 'testCases'],
      });
