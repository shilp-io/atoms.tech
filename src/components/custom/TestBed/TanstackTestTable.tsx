        ...
        <div>
                                    ...
                                    {row.getVisibleCells().map((cell, index) => (
                                        <TableCell 
                                            ...
                                            className={cn(
                                                ...
                                                index === row.getVisibleCells().length - 1 && "relative"
                                            )}
                                        >
                                            {flexRender(
                                                    ...
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            <DropdownMenuItem
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    (table.options.meta as TableMetaType)?.onEdit(row.original);
                                                                }}
                                                            >
                                                                Edit
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem
                                                                ...
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    (table.options.meta as TableMetaType)?.onDelete(row.original.id || '');
                                                                }}
                                                            >
                                                                Delete
                                                            </DropdownMenuItem>
            ...
            <div className="flex items-center justify-between py-4 px-4">
                <div className="flex-1 text-sm text-muted-foreground">
                    Showing {pageSize === data.length ? 'All' : `${pageIndex * pageSize + 1} - ${Math.min((pageIndex + 1) * pageSize, data.length)}`} of {data.length} entries
                </div>
...
export const columns: ColumnDef<TestReq>[] = [
    {
        accessorKey: 'test_id',
        header: 'Test ID',
        cell: ({ row }: { row: Row<TestReq> }) => row.original.test_id?.substring(0, 6) || '',
        enableSorting: true,
        size: 100,
    },
    {
    ...
    },
    {
        ...
        cell: ({ row, table }: { row: Row<TestReq>; table: TanstackTable<TestReq> }) => {
            ...
            return (
                <span className={(table.options.meta as TableMetaType)?.getPriorityStyle(priority) || ''}>
                    {priority}
                </span>
            );
        },
    },
    {
        ...
        cell: ({ row }: { row: Row<TestReq> }) => {
            ...
            if (result === 'Pass') return <span className="text-green-600 dark:text-green-400">Pass</span>;
            if (result === 'Fail') return <span className="text-red-600 dark:text-red-400">Fail</span>;
