...
import { BlockPropertySchema, DocumentPropertySchema } from '../types';
import { useCreateBlockPropertySchema, useDocumentPropertySchemas } from '@/hooks/queries/usePropertySchemas';
...
export const useTableBlockActions = ({
  ...
  const { data: documentPropertySchemas, isLoading: isLoadingDocSchemas, isError: isDocSchemasError } = useDocumentPropertySchemas(documentId);
  ...
  const createBlockPropertySchemas = async () => {
    ...
    try {
      ...
      if (!documentPropertySchemas || documentPropertySchemas.length === 0) {
      ...
      const createdSchemas = await Promise.all(
        documentPropertySchemas.map(async (docSchema) => {
          ...
          return await createBlockPropertySchemaMutation.mutateAsync(blockSchema);
        })
      );
  ...
  const createDefaultDocumentPropertySchemas = async (
    createDocumentPropertySchema: (schema: Partial<DocumentPropertySchema>) => Promise<DocumentPropertySchema>
  ) => {
    if (!userProfile?.id) {
      throw new Error('User profile not found');
    }
