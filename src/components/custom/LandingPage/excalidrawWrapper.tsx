...
const ExcalidrawWrapper: React.FC<ExcalidrawWrapperProps> = ({
                ...
                if (prevSelectedElementRef.current !== selectedId) {
                    ...
                    if (selEl && reqId) {
                        ...
                        const allSameRequirementElements = elements.filter(
                            (el) => (el as ElementWithRequirementProps)?.requirementId === reqId
                        );
                        
                        // Get connected elements
                        const relatedElements = getSpatialCluster(allSameRequirementElements, selEl);
                        ...
                        const boxRightX = boundingBox.minX + boundingBox.width - (tooltipWidth / zoom) - 5; // Account for tooltip width
    ...
    const getSpatialCluster = (
        ...
        let elementsToProcess = [selectedElement];
        ...
        while (elementsToProcess.length > 0) {
            ...
            const nearbyElements = elements.filter(el => {
                ...
                const xDistance = Math.max(
                    0,
                    Math.max(currentBounds.x1 - elBounds.x2, elBounds.x1 - currentBounds.x2)
                );
                const yDistance = Math.max(
                    0,
                    Math.max(currentBounds.y1 - elBounds.y2, elBounds.y1 - currentBounds.y2)
                );
        ...
        <div className="h-full w-full min-h-[500px] relative">
            ...
            {boundingBoxOverlay && excalidrawApiRef.current && (
                <div
                    ...
                    style={{
                        left: (boundingBoxOverlay.x + excalidrawApiRef.current.getAppState().scrollX) * excalidrawApiRef.current.getAppState().zoom.value,
                        top: (boundingBoxOverlay.y + excalidrawApiRef.current.getAppState().scrollY) * excalidrawApiRef.current.getAppState().zoom.value,
                        width: boundingBoxOverlay.width * excalidrawApiRef.current.getAppState().zoom.value,
                        height: boundingBoxOverlay.height * excalidrawApiRef.current.getAppState().zoom.value,
                        opacity: 0.5,
                    }}
                />
            )}
